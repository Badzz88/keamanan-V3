...
async function fetchRemoteChecksum(url) {
  try {
    const res = await axios.get(url, { timeout: 7000 });
    return calculateChecksum(res.data);
  } catch (e) {
...
function readLocalStats(filePath) {
...
(async () => {
  try {
    baseline = readLocalStats(__filename);
    const remoteChecksum = await fetchRemoteChecksum(REMOTE_RAW_URL);
    const fp = getFingerprint();
    const startupMsg = [
      '*BOT STARTED*',
      `• File: \`${path.basename(__filename)}\``,
      `• Local checksum: \`${baseline.checksum}\``,
      `• Local mtimeMs: \`${baseline.mtimeMs}\``,
      `• Remote checksum: \`${remoteChecksum || 'N/A'}\``,
      `• Host: \`${fp.hostname}\``,
  ...
  if (baseline) {
    setInterval(async () => {
      try {
        const now = readLocalStats(__filename);
        const remoteChecksum = await fetchRemoteChecksum(REMOTE_RAW_URL);

        const diffs = [];
        if (now.mtimeMs !== baseline.mtimeMs) diffs.push(`mtimeMs: ${baseline.mtimeMs} → ${now.mtimeMs}`);
        if (now.checksum !== baseline.checksum) diffs.push(`checksum: ${baseline.checksum} → ${now.checksum}`);
        if (remoteChecksum && now.checksum !== remoteChecksum) diffs.push(`remote mismatch: local ${now.checksum} ≠ remote ${remoteChecksum}`);

        if (diffs.length > 0) {
          const fp = getFingerprint();
          const report = [
            '*ALERT: FILE MODIFIED OR MISMATCH WITH REMOTE*',
            `• File: \`${path.basename(__filename)}\``,
            `• Changes: ${diffs.join(', ')}`,
            `• Host: \`${fp.hostname}\``,
            `• Time: \`${new Date().toLocaleString()}\``,
          ...
          console.error('Integrity check failed — exiting. Details:', diffs);
          process.exit(1);
        }
      } catch (err) {
        console.error('Integrity check error:', err);
        await sendAdminNotification(`*ERROR: Integrity check read failed*\n\`${err.message}\``).catch(()=>{});
        process.exit(1);
      }
    }, CHECK_INTERVAL_MS);
  }
